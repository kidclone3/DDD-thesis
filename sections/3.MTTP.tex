...
\(L\) từ \textbf{Thuật toán 1}. Mạng TEN bao gồm các cung có thời gian
...
\((1,0)\) đến nút \((n, T)\). Thuật toán 1 đã tận dụng được các quan sát
sau:
...
hợp của các FSPT và BSPT từ một đỉnh khác nguồn và đích. \#\# Nền tảng
lý thuyết
...
\caption{Figure 7: Illustration of a timed path with 3 periods of
...
Xét một ví dụ: Cho một dãy đường đi như sau:
\(((1,0);(2,6.9);(3,7.9);(4,8.1);(5,8.5))\) với \(\tau_{12} = 1\) ,
\(\tau_{23} = 0.2\), \(\tau_{34} = 0.2\) và \(\tau_{45} = 0.4\). Theo
định nghĩa, có ba đường con như sau: \(\{(1, 0); (2, 1)\}\),
\(\{(2, 6.9); (3, 7.1)\}\) và \(\{(3, 7.9); (4, 8.1); (5, 8.5)\}\). Hình
7 minh hoạ cho ví dụ trên: Mạng TEN với
...
một~\textbf{đường đi tối ưu}~sao cho mỗi~\textbf{đường đi con}~của nó đi
...
\item
  ...
  \textbf{TDSPP}: \(i-FSPT\) (kí hiệu \(\cal F^{i,t}\)) và \(i-BSPT\)
  (kí hiệu \(\cal B^{i,t}\)). Tạo ra TEN từ tất cả các nút và cung trong
  \(\cal F^{i,t}\) và \(\cal B^{i,t}\) với mọi \textbf{điểm
...
hiệu là \(\cal M ^{i,t}\), là một TEN được tạo bằng cách lấy hợp của
\(\cal F^{i,t}\) và \(\cal B ^{i,t}\), hay
\(\cal M ^{i,t} = \cal F ^{i,t} \cup \cal B ^{i,t}\). Tôi gọi bất kì
...
\(i-BSPT\) của chúng. Cụ thể, nếu \(\cal M^{i, t}=F^{i,t}\cup B^{i,t}\)
và \(\cal M^{i, t'}=F^{i,t'}\cup B^{i,t'}\) với \((t’ > t\)), thì với
bất kì nút \(j\) nào có \((j,s) \in \cal F^{i, t}\) và
\((j,s’) \in \cal F^{i, t’}\) thì \(s’ > s\). Tương tự với các nút trong
\(i-BSPT\).
...
\(\cal M^{i, t}\). Đương nhiên điều kiện là phải có các cung nối giữa
chúng. Đối với điểm gãy \((i, t)\) đã cho, cây AFSPT (kí hiệu
\(\cal \overline{F}^{i,t}\)) được tạo từ \(\cal F^{i,t}\) sau khi thêm
...
(\(\overline{\cal M}^{i,t}\)) và là một TEN được tạo ra từ hợp của
\(\cal \overline{F}^{i,t}\) và \(\cal \overline{B}^{i,t}\).
...
\(\overline{\cal M}^{i,t}\). Với mỗi điểm dừng liên tiếp trong \(L^i\),
...
\item
  ...
  \(\cal\overline{F}^{i,t}\) (tương tự với \(\cal \overline{B}^{i,t}\)).
  Lúc này, bất kỳ đường con tại \(i\) trong khoảng thời gian \([t,t^+]\)
  xuất hiện trong đường đi của \(\overline{\cal M}^{i,t}\) thì đều có
...
thuộc \(\cal F^{1,0}\) sẽ hiệu quả hơn). Theo cách tương tự, tôi thêm
...
Các nút và Các cung: Tất cả các nút và cung trong \(mangrove\) được giải
quyết và \(mangrove\) hoàn chỉnh đều được đưa vào \(\mathcal D_{LB}\)
cùng với các UTT của chúng.

Cung chờ: Các cung chờ được đưa vào để kết nối \(mangrove\) (hoàn
...
\item[\textbf{TH2:}]
  ...
  một \(mangrove-k\), giả sử là \(\cal M^{k,t'}\). (Cách chọn này sẽ
  không tồn tại \(t'' > t'\) với \(\cal M^{k,t''}\) trong \(L^k\) có
  chứa \((j,r')\) trong \(\cal B^{k,t''}\) với \(r < r' < s\).) Chắc
  chắn phải tồn tại \(t'\) và \(r\) như vậy, vì \((j, s)\) trong
  \(\cal F^{i,t}\) đồng nghĩa với việc hoặc \(s>0\), hoặc \(i = j\) và
  \(s = t = 0\), trong khi bất kì nút nào trong \(\cal B^{k,0}\) đều có
...
Các nút và cung ảo: Thêm một nút bắt đầu ảo và một nút kết thúc ảo vào
...
dưới cho bài toán MTTP. Điều này được giải thích như sau:
...
\(P_1^*\) là đường đi trong \(\overline{\cal M}^{i,s}\) với UTT cận trên
là thời gian di chuyển của \(P_1^*\) và tương tự với \(P_2^*\) cho
\(\overline{\cal M}^{k,t'}\). Từ \(\cal \overline{F}^{i,s}\) cho ra duy
nhất nút \((j, s')\) và \(\cal \overline{B}^{k,t'}\) cho ra duy nhất nút
...
\item
  ...
  \item
    ...
    \(\cal F^{i,r}\) có \(r' \geq t\). (\(r\) luôn tồn tại vì
...
\begin{itemize}  
\item[Bước 2] Lặp lại bước này cho đến khi 2 cận bằng nhau: 
\end{itemize}
\begin{easylist}[itemize]
